  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (BuildContext context) =>
          GetItScope.of(context).get<ProfileFavoriteBloc>(
...
class _ProfileFavoriteTabContent extends StatelessWidget {
  ...
  List<Widget> _buildSector(
      BuildContext context, FavoriteType type, List<FavoriteItemModel> items) {
    return [
      if (items.isNotEmpty)
        SliverToBoxAdapter(
          child: Padding(
          ...
          ? SliverToBoxAdapter(
              child: Wrap(
                ...
                children: items
                    .map(
                      (e) => OutlinedButton(
                        onPressed: () => RootRouterDelegate.get()
                            .navigateToDetailStudio(e.id),
                        child: Text(e.title),
                      ),
                    )
                    .toList(),
